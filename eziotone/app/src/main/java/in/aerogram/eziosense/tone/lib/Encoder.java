...
import in.aerogram.eziosense.tone.lib.utils.ArrayUtils;

import static in.aerogram.eziosense.tone.lib.Constants.kAmplitude;
import static in.aerogram.eziosense.tone.lib.Constants.kBitsPerByte;
import static in.aerogram.eziosense.tone.lib.Constants.kBytesPerDuration;
import static in.aerogram.eziosense.tone.lib.Constants.kSamplesPerDuration;
import static in.aerogram.eziosense.tone.lib.Constants.kSamplingFrequency;
        ...
        for (int j = 0; j < kBytesPerDuration; j++) {
            for (int k = 0; k < kBitsPerByte; k++) {
                ...
                double innerMultiplier = getFrequency((j * kBitsPerByte) + k)
                        * (1 / kSamplingFrequency) * 2 * Math.PI; // TODO
                for (int l = 0; l < signal.length; l++) {
                    signal[l] = signal[l] + (kAmplitude * Math.cos(innerMultiplier * l)); // TODO
                }
            }
        }
